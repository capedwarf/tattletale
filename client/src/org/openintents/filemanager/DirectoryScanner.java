...
public class DirectoryScanner extends Thread {
    ...
    public void run() {
        ...
        File[] directories = currentDirectory.listFiles(fileFilter);
        File[] photos = currentDirectory.listFiles(filter);
        int sizeDir = (directories == null) ? 0 : directories.length;
        int sizePhotos = (photos == null) ? 0 : photos.length;
        File[] files = new File[sizeDir + sizePhotos];
        if (directories != null){
            System.arraycopy(directories, 0, files, 0, sizeDir);
        }
        if (photos != null){
            System.arraycopy(photos, 0, files, sizeDir, sizePhotos);
        }
        int fileCount = 0;
        int dirCount = 0;
        int sdCount = 0;
        ...
        if (files != null) {
            Log.d(TAG, "FIles sum = " + files.length);
            // Timer.start(); // TODO?
            for (File currentFile : files) {
                ...
                //Timer.printTime("One file scanned: ");
            }
        }
        //Timer.printTime("ALL files scanned: ");
    ...
    private class ExtensionsFilter implements FilenameFilter {
